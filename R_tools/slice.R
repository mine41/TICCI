...
#'    \item{\code{ident}:}{\code{"vector"}, the identity (cluster/state/type) of single cells  }
...
#'    \item{\code{vds}:}{\code{"data.frame"}, a data frame containing the x and y coordinates of each cell for visualization  }
...
#'
#' construct and initialize a slice object
#'
...
#' @param cellidentity A vector encoding the original identity of cells (e.g., time information or cell types); if not set, default to a vector of "cell"
#' @param projname A string describing the analysis; if not set, default to "SLICE-" with timestamp information
...
#' @param km A symmetric matrix encoding the functional similarity of genes; rownames and colnames must be official NCBI gene symbols.
#' @param calculation Choose the "bootstrap" or "deterministic" calculation of scEntropy
#' @param exp.cutoff The minimum value of a gene to be considered expressed 
#' @param B.size In bootstrap calculation, specify the size of bootstrap sample; in deterministic calculation, specific the number of top expressed genes from each cell
...
#'
#' reduce expression space using PCA
#'
#' @param object slice object
...
#' 
#' Prerequisite: the "vds" and "rds" slots in the input slice object must be set
#'
#' @param object Slice object 
...
#' @param reverse If TRUE, infer differentiation lineage model (edge directions follow entropy reduction), if FALSE, infer dedifferentiation lineage model (edge directions follow entropy induction)
...
#'                         Only take effect when the graph-based method is selected
...
#'                         Only take effect when the graph-based method is selected.
#' @param cluster.method Use "kmeans" or "pam" to divide cells into clusters. Only take effect when the clustering-based method is selected. 
...
setGeneric("getLineageModel", function(object, lm.method="clustering", model.type="tree", reverse=F, ss.method="all", ss.threshold=0.25, # common parameter
                                       ...
                                       context_str="", ...) standardGeneric("getLineageModel"))
#' @export
...
#'
#' @param object Slice object
#' @param method Use either "sp" (shortest-path based) method or "pc" (principal-curve based) method to reconstruct cell trajectories following a lineage in the infered lineage model 
...
#' @param NN.threshold Used in the shortest path based method, specifying the distance threshold for determining neighbors of each cell in the path. Default is 0.7 quantile of all edge weights in the network 
...
#' 
#' for each lineage, the expression profile of each gene will be first smooth using cubic regression splines, and then fit to two models (M1: changes in the lineage; M0: no change)
#' likelihood ratio test will be used to compare the goodness of fit of the two models and assigns a p-value to each gene
#' Only genes meet the following criteria will be selected for diff test: expression >=thresh.exp in at least thresh.cell cells and variance>=thresh.var  
#'
...
#' 
#' Given the M1 predicted expression profiles of differentially expressed genes in a transition, the function uses PAM clustering to divide differentially expressed genes into patterns  
#'
#' @param expr The M1 predicted expression profiles of differentially expressed genes in a transition
#' @param do.zscore Whether to perform gene based zscore transformation prior to clustering
#' @param c.method The algorithm to find pattern clusters, currently support "pam" and "kmeans" 
#' @param k The number of patterns to be discovered
#' @param d.method The distance method
#' @param do.plot Whether to plot discovered patterns
#' @param plot.xlabel The string for the label of x axis of the plot
#' @param plot.ylabel The string for the label of y axis of the plot
#' @param plot.filename The file name of the plot
#' @param plot.w The width of the plot
#' @param plot.h the height of the plot
#' @return An object of class "pam" or "kmeans" representing the clustering results.
#' @export
getPatterns <- function(expr, do.zscore=T, c.method="pam", k=NULL, d.method=function(x) as.dist((1 - cor(t(x)))/2), 
                        do.plot=T, plot.xlabel="Transitional Path", plot.ylabel="Expression (+1, log2)", plot.filename=NULL, plot.w=3, plot.h=4, context_str="") {
